...
def make_music_video(qc, name, rhythm, noise_model = None, input_instruments = [list(range(81,89))], note_map = note_map_chromatic_middle_c, invert_colours = False, fps=60, vpr = None, smooth_transitions = True, phase_marker = True, synth="timidity", log_to_file = False, probability_distribution_only = False):
    ...
    base_output_filename = os.path.join(output_path, name)

    # Remove any existing wav files
    try:
        wav_files = glob.glob(base_output_filename + '*.wav')
        for file in wav_files:
            os.remove(file)
    except:
        pass

    files = glob.glob(base_output_filename + '-*.mid')
    if len(files) == 0:
        files = glob.glob(base_output_filename + '.mid')

    if synth.lower() == "timidity":
        musical_processing.convert_midi_to_wav_timidity(files, timeout=8, log_to_file=log_to_file)
    ...
    files = glob.glob(base_output_filename + '-*.wav')
    if len(files) > 1:
        print("Combining tracks into a single .wav file...")
        musical_processing.mix_wav_files(files, base_output_filename)
...
def make_music_midi(quantum_circuit, name, rhythm, noise_model = None, input_instruments = [list(range(81,89))], note_map = note_map_chromatic_middle_c, separate_audio_files = True):
    ...
    output_manager = data_manager.DataManager("./" + name)

    if noise_model == None:
        noise_model = NoiseModel()

    density_matrices_pure = quantum_simulation.sample_circuit_barriers(quantum_circuit)    
    density_matrices_noise = quantum_simulation.sample_circuit_barriers(quantum_circuit, noise_model)
    
    # calculate the fidelity of each of the sampled quantum states
    fidelity_list = [qi.state_fidelity(dm_noise, density_matrices_pure[i]) for i, dm_noise in enumerate(density_matrices_noise)]
    ...
    output_manager.save_json(sounds_list, "sounds_list.json")
    output_manager.save_json(fidelity_list, "fidelity_list.json")
    output_manager.save_json(measurement_probabilities_samples, "meas_probs_list.json")
    output_manager.save_json(rhythm, "rhythm.json")
    ...
    for t_i, sound in enumerate(sounds_list):    
        ...
        for trackno in range(numtracks):
            ...
            if trackno >= len(sorted_chords):
                ...
                active_notes.append((trackno,notes_by_basis_state_index, channel))
                continue
            ...
            for n in chord:
                ...
                track.append(Message('note_on', channel=channel, note=notes_by_basis_state_index, velocity=vol_128, time=0))
                active_notes.append((trackno, notes_by_basis_state_index, channel))
        ...
        for trackno, track in enumerate(tracks):   
            if separate_audio_files == True:
                ...
                channel = trackno
            track.append(Message('note_on', channel=channel, note=0, velocity=0, time=rhythm[t_i][0]))
        ...
        for trackno, notes_by_basis_state_index, channel in active_notes:
            track = tracks[trackno]
            track.append(Message('note_off', channel=channel, note=notes_by_basis_state_index, velocity=0, time=0))
        ...
        for trackno, track in enumerate(tracks): 
            if separate_audio_files == True:
                ...
                channel = trackno
            track.append(Message('note_on', channel=channel, note=0, velocity=0, time=rhythm[t_i][1]))
            track.append(Message('note_off', channel=channel, note=0, velocity=0, time=0))
...
def make_video(qc, name, rhythm, noise_model = None, input_instruments = [list(range(81,89))], note_map = note_map_chromatic_middle_c, invert_colours = False, fps=60, vpr = None, smooth_transitions = True, phase_marker = True, separate_audio_files = True, probability_distribution_only = False):
    ...
    NAME = name
    target_folder = NAME
    # TODO: use working directory, since file in folder is not found for some reason
    ...
    def plot_quantum_state(input_probability_vector_list, angle_vector_list, meas_prob_vector_list, plot_number, interpolate = False, save=True, fig = None, zero_noise = False, probability_distribution_only = probability_distribution_only):
        ...
        if interpolate == True:
            ...
            for i in range(input_probability_vector_1.shape[0]):
                input_probability_vector[i, :] = video_generation.lerp(input_probability_vector_1[i, :], input_probability_vector_2[i, :], plot_number - math.floor(plot_number))
            ...
            for i in range(angle_vector.shape[0]):
                angle_vector[i, :] = video_generation.lerp(angle_vector_1[i, :], angle_vector_2[i, :], plot_number - math.floor(plot_number))
            ...
            meas_prob_vector = video_generation.lerp(meas_prob_vector_1[:], meas_prob_vector_2[:], plot_number - math.floor(plot_number))
    ...
    barrier_only_clip_mask = barrier_only_clip_mask.fl_image( lambda pic: video_generation.filter_colour_round_with_threshold(pic, threshold=0.9, colour_dark=0.0, colour_light=1.0))
    ...
    barrier_only_clip = barrier_only_clip.fl_image( lambda pic: video_generation.filter_color_blend(pic, [255, 255, 255], 0.3))
    ...
    def draw_needle(get_frame, t):
        ...
        lerp_colour = [int(x) for x in video_generation.ease_out(highlight_colour, idle_colour, lerp_time)]
